...
export default function RailwayNetwork() {
  ...
  useEffect(() => {
    ...
    // 3. Draw nodes or clusters as custom Leaflet markers
    nodesToRender.forEach(item => {
      if (item.type === 'node') {
        ...
        const clusterStatus = getClusterStatus(cluster.nodes);
        const markerIcon = L.divIcon({
          className: '',
        ...
        marker.on('click', () => {
          const currentZoom = mapInstanceRef.current ? mapInstanceRef.current.getZoom() : 5;
          mapInstanceRef.current?.setView([cluster.center.lat, cluster.center.lng], Math.min(currentZoom + 2, 18));
        });

        const tooltipContent = cluster.nodes.map(n => n.name).slice(0, 5).join(', ') + 
          (cluster.nodes.length > 5 ? ` and ${cluster.nodes.length - 5} more` : '');
        
        marker.bindTooltip(`Cluster: ${tooltipContent}`, {
          permanent: false,
          direction: 'top',
          className: 'custom-tooltip',
          offset: [0, -10]
  ...
  }, [nodesToRender, visibleRoutes, visibleCorridors, selectedNode, mapMode, filteredNodes, zoom, highlightedRouteIds, nodeMap]);
  ...
  useEffect(() => {
    ...
    visibleRoutes.forEach(route => {
      const routeCoords = route.coordinates?.length >= 2
        ? route.coordinates
        : resolveRouteCoords(route);
      if (routeCoords.length < 2) return;
  ...
  }, [visibleRoutes, transitNodes, isRunning, mapMode, nodeMap]);
            ...
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {[
                  ...
                  ['Connections', <span key="conn" style={{ fontWeight: 'var(--font-bold)' }}>{selectedNode.connectionCount ?? nodeConnections.length}</span>],
                  ...
                  ['Linked Routes', <span key="routes" style={{ fontSize: 'var(--text-xs)', color: 'var(--text-secondary)' }}>{nodeConnections.slice(0, 3).map((r) => r.routeCode).join(', ')}{nodeConnections.length > 3 ? ` +${nodeConnections.length - 3}` : ''}</span>],
